"""Core scoring engine — bloom gate (zero LLM cost).

Determines if detector signals contain enough extractable insights to bloom.
Uses InsightExtractor (rule-based) for COUNTING only — user-facing content
is generated by InsightGenerator (LLM) after bloom is triggered.
"""
